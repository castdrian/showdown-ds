...
verify_thor_layout_patch() {
    ...
    primary_y_count="$(rg -c 'primary->second\.pos_y = 0;' "$overlay_patch_file" 2>/dev/null || true)"
    ...
    lower_input_y_origin_count="$(rg -c '^\+\s+pos_y = iter\.second\.pos_y;' "$overlay_patch_file" 2>/dev/null || true)"
