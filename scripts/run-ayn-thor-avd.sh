...
verify_thor_layout_patch() {
    ...
    lower_x_count="$(rg -c 'thorDisplay->second\.pos_x = \(primary->second\.originalWidth - thorPreviewWidth\) / 2;' "$overlay_patch_file" 2>/dev/null || true)"
    ...
    lower_input_scale_count="$(rg -c 'iter\.second\.originalWidth / iter\.second\.width' "$overlay_patch_file" 2>/dev/null || true)"
    if [[ "$upper_y_count" != "2" || "$lower_y_count" != "2" || "$lower_x_count" != "2" || "$lower_width_count" != "2" || "$lower_height_count" != "2" || "$lower_input_scale_count" != "1" ]]; then
