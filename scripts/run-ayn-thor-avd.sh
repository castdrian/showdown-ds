...
verify_thor_layout_patch() {
    ...
    lower_input_y_transform_count="$(rg -c '^\+\s+pos_y = totalH - iter\.second\.height - iter\.second\.pos_y;' "$overlay_patch_file" 2>/dev/null || true)"
    if [[ "$primary_y_count" != "3" || "$lower_y_count" != "3" || "$lower_x_count" != "3" || "$lower_width_count" != "3" || "$lower_height_count" != "3" || "$lower_input_x_scale_count" != "1" || "$lower_input_y_scale_count" != "1" || "$lower_input_y_transform_count" != "1" ]]; then
