...
class BattleSpriteRequestTest {
    ...
    fun modernSpriteCacheNeverFallsBackToLegacyLocalSprites() {
        ...
        val combinedBackIndex = source.indexOf("requestHdBackOrRegularResolution(request, plan, includeRegularScrapedBack, callback)", backIndex)
        ...
        val combinedResolver = source.substringAfter("private fun requestHdBackOrRegularResolution")
        ...
        val communityIndex = source.indexOf("requestAnimatedSpriteCandidates(plan.communityRemoteCandidates.take(MAX_COMMUNITY_SPRITE_CANDIDATES))", frontIndex)
        ...
        assertTrue(combinedBackIndex >= 0)
        assertTrue(backLocalIndex >= 0)
        assertTrue(combinedResolver.contains("requestNumberedHdSpriteResolution(request)"))
        assertTrue(combinedResolver.contains("requestRegularRemoteSpriteResolution(plan)"))
        assertTrue(backPreferredIndex < scrapedBackIndex)
        assertTrue(scrapedBackIndex < combinedBackIndex)
        assertTrue(combinedBackIndex < backCommunityIndex)
        assertTrue(combinedBackIndex < backLocalIndex)
