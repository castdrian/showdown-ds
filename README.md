...
The launcher uses a resource-safe low-RAM debug profile: 1 virtual CPU, 1024 MB of guest RAM, a 128 MB VM heap, a 30 Hz host frame cap, and host Vulkan on macOS. This keeps API 34 viable for dual-screen app installation and launch without reserving a large share of a 16 GB development machine. The low-RAM emulator flag prevents API 34 from silently inflating the guest allocation to its normal phone profile. CPU, guest RAM, and VM heap overrides are bounded at 2 cores, 2048 MB, and 256 MB so a diagnostic launch keeps its configured guest allocations bounded. Additional emulator arguments are rejected so raw QEMU options cannot bypass those limits. The guest still renders at the Thor's native display sizes and calibrated densities; the host emulator window does not change those guest metrics. Set `AYN_THOR_CPU_CORES`, `AYN_THOR_RAM_MB`, or `AYN_THOR_HEAP_MB` within those limits, set `AYN_THOR_VSYNC_RATE=120` when validating the upper display at its target refresh rate, or use `AYN_THOR_GPU_MODE` to select a different graphics backend when needed.
